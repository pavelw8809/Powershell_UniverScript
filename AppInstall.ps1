...
.DESCRIPTION
    EXTENDED NAME:
    Main installation function (with checking if the software is installed)

    DESCRIPTION:
    Main installation function for all installers/processes
    Takes installation arguments, checking if the software is installed and run InstallCMD function.
...
.PARAMETER App
    ...
    - full path to main exe file
    - msiexec.exe - in case of msi installers

.PARAMETER AppArgs
    Argumets to installation command.
    - All installation switches
...
.EXAMPLE
    Run msi with defalt timeout of 60 minutes
    AppInstall -GUID "{1FEAA855-A899-40DE-8E9B-F715DD956E95} -App "msiexec.exe" -AppArgs "/i "C:\JavaConf.msi /qn /l*v "C:\O-I\logs\JavaConf_1.0.log" -AppName "Java Configuration 1.0"
    AppInstall "{1FEAA855-A899-40DE-8E9B-F715DD956E95} "msiexec.exe" "/i "C:\JavaConf.msi /qn /l*v "C:\O-I\logs\JavaConf_1.0.log" "Java Configuration 1.0"

.EXAMPLE
    Run exe with timeout extended to 120 minutes
    AppInstall -GUID "SAPGUI" -App "C:\SAP_GUI_exe" -AppArgs " /install /quiet" -AppName "SAP GUI For Windows 7.40" -Time 120
    AppInstall "SAPGUI" "C:\SAP_GUI_exe" -AppArgs " /install /quiet" "SAP GUI For Windows 7.40" 120

.EXAMPLE
    Run exe with default timeout of 60 minutes
    AppInstall -GUID "Pulse Secure 9.1" -App "C:\PulseInstaller.exe" -AppArgs "/s /v"qn"" -AppName "Pulse Secure 9.1"
    AppInstall "Pulse Secure 9.1" "C:\PulseInstaller.exe" "/s /v"qn"" "Pulse Secure 9.1"
...
Function AppInstall {
    ...
    if(!($Reg64 -Or $Reg32)) {
        Write-Log -type 4 -msg "Start $AppName installation"
